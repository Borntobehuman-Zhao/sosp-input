...
//	}
	...
	//加载本文件样式
	try{
		initFileStyle(layout.dbdStyles);
	}catch(e){}
}
...
function redefineFrameHeight(n){
	var newHeight = parseInt($('iframe[confName='+n+']').parent().css('height'))-divide -20;//减去area里bar和一点偏移
...
function createMenuBar(reportName, areaDom){
	...
	$(areaDom).append(bar);
	var barinner = $('<div class="rmbar row-fluid box-header well" id="rmbar_'+reportName+'"></div>')
	bar.append(barinner);
	...
	barinner.append('<div style="float:right" class="dontchoose"></div>');
	var divs = barinner.find('div');
	$(divs[2]).append('<div class="dontchoose" style="float:left"></div>');
	$(divs[2]).append('<div class="dontchoose" style="float:left"></div>');
	$(divs[2]).append('<div class="dontchoose" style="float:left"></div>');
	$(divs[2]).append('<div class="dontchoose" style="float:left"></div>');
	$(divs[0]).append('<div class="barReportName barReportTitle dontchoose" id="barReportTitle_'+reportName+'" value="'+reportName+'">'+reportName+'</div>');
	...
	if(!previewDbd) {
		...
		$(div1).append('<div title="转为控件" class="toEditor" onclick="javascript:toEditor(\''+$(areaDom).attr('id')+'\');"><img style="height:16px" src="../img/guide/dashboard/editor.png"/></div>');
		$(div2).append('<div title="转为报表" class="toReport"  onclick="javascript:toReport(\''+$(areaDom).attr('id')+'\');"><img style="height:16px" src="../img/guide/dashboard/table.png"/></div>');
		$(div3).append('<div title="报表参数" class="reportWhere" style="cursor:no-drop"><img src="../img/guide/dashboard/filter.png"/></div>');
		$(div4).append('<a title="删除" class="reportRemove" href="javascript:removeReport(\''+reportName+'\');"><img src="../img/guide/13.png"/></a>');
	}
...
function initEditors(){
	...
	for(var i = 0 ; i < areas.length; i++){
		...
		toEditor('singleArea'+area.id,true);
		if(!p1.pname) continue;
		createEditor(p1);
	}
}
