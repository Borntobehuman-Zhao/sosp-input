...
function reportCellClick(event) {
	//dbd暂不开放
	//jQuery_pf(event.target).powerFloat({target:$('#pickValueFloat'),eventType:'click',zIndex:50000});
	var upperWindow = window.parent;
	if(upperWindow.editor_global_data.picker){
		var td = this;
		if(td.tagName == "TD" && $(td).attr('tt') == "4"){
			upperWindow.editor_global_data.dealPicker(td.innerHTML);
			$('html').css('cursor','auto');
			upperWindow.document.getElementsByTagName('html')[0].style.cursor = 'auto';
		}
	}
	return;
