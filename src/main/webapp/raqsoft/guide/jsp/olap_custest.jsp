...
if (dataSource.length() == 0 
	...
	//fixedTable="客户";
	//ql = "select * from 雇员 where 雇员ID = ${雇员ID}";
}
...
system.out.println(oldFilters == null);
String decompressed = "";
String inssql = request.getParameter( "inssql" );
 System.out.println("解压前的sql:"+inssql);
//inssql=inssql.replace("_","+");
//System.out.println("解压前的sql:"+inssql);
if(inssql != "" && inssql != null){
	inssql=inssql.replace("_","+");
	System.out.println("解压前的sql:"+inssql);
	ByteArrayOutputStream out1 = null;
	        ...
	        try {
	            ...
	            byte[] compressed = new Base64().decode(inssql);
	            ...
	            while ((offset = gin.read(buffer)) != -1) {
	                out1.write(buffer, 0, offset);
	            }
	            decompressed = out1.toString("utf-8");
	        } catch (IOException e) {
	        	e.printStackTrace();
	            decompressed = null;
	        } finally {
	        ...
	        System.out.println("解压后的sql:"+decompressed);
}

//String encodedsqlText = "select * from fccont where contno='AH034808' ";

//如果之前有条件了，而且这次访问没有拼vsb参数，那么用老条件
if(oldFilters != null && oldFilters.size()>0 && (vsb == null || vsb.length() == 0) && norefreshfilter.equals("yes")){
	filters = oldFilters;
}else {
	system.out.println("---set new filters---");
	system.out.println(session().getId());
	...
	//f.getParamValues().put("雇员ID","6");//设置过滤条件里的参数值（宏值）
	
	f.getParamValues().put("encodedsqlText",decompressed);//设置过滤条件里的参数值（宏值）
...
guideConf.filter = "encodedsqlText";
	/*这些参数可以控制页面的初始状态
	...
	*/
	guideConf.showToolBar = 'no';//是否显示上部工具条
	guideConf.infinity="0";  //对无穷大的显示
	guideConf.showZongji = "no";//报表里是否显示总计
	guideConf.autoReloadDimDataOnServer = 'yes';
	...
	/*var sqlDatasets = [
		{
			...
			,sql:"SELECT T0.(客户) 客户 ,T0.(客户名称) 客户名称 ,T0.(联系人姓名) 联系人姓名 ,T0.(联系人职务) 联系人职务 ,T0.(市) 市 ,T0.(客户IDs) 客户IDs ,T0.(VIP级别) VIP级别 ,T0.(VIP折扣率) VIP折扣率 ,T0.(VIP起始时间) VIP起始时间 ,T0.(VIP结束时间) VIP结束时间 FROM VIP客户 T0" //如果服务器管理这些，js里就不用定义
			,fields:null
		}
	]*/
