...
if(oldFilters != null && oldFilters.size()>0 && (vsb == null || vsb.length() == 0) && norefreshfilter.equals("yes")){
	...
	f.getParamValues().put("name",request.getParameter("name"));//设置过滤条件里的参数值（宏值）
}


boolean isBig = false;
session.setAttribute("isBig",isBig?"1":"2");//1大报表，2普通报表
String reportPage = null;
if (isBig) reportPage = cp+"/reportJsp/showOlapBigReport.jsp";
else reportPage = cp+"/raqsoft/guide/jsp/showReport.jsp";
%>
...
<script>
/* function refreshOne(reportName){
	//queryDataSet : function(reQuery,dataSet,callback)
	var dsx = rpxu.getRpx(reportName).dataSet;
	aly.queryDataSet("1", dsx,function(d){
		console.log(reportName);
		if(d.trim() == "ok") aly.refreshReport(reportName);
	});
}

function refreshAll(){
	for (var i=0; i<rqAnalyse.rpxs.length; i++) {
		refreshOne(rqAnalyse.rpxs[i].name);
	}
}
$(function(){
	$('body').append('<a style="top:200px;position:fixed" href="javascript:refreshAll();">refresh</a>');
}); */
...
//guideConf.dataFolderOnServer = "/WEB-INF/files/data/";//服务器上缓存数据文件的根目录，默认/WEB-INF/files/data/
	...
	guideConf.filter="default";
	guideConf.alwaysRefreshCache = true;//type < 6时候用txt或bin缓存文件，需要强制重查时候用这个配置，使用url参数时尤其重要
	*/
	guideConf.dataFileType = "<%=dataFileType%>";
	guideConf.reportPage = "<%=reportPage%>";
	guideConf.autoReloadDimDataOnServer = 'yes';
	...
	var sqlDatasets = [
		{
			sqlId:"s1"
			,dataSource:"mysql" //如果服务器管理这些，js里就不用定义
			,sql:"select * from t1" //如果服务器管理这些，js里就不用定义
			,fields:null
		}
	]
