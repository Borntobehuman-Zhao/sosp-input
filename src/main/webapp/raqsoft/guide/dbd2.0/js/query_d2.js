...
//		$('body').css('overflow','auto')
		if (olapStr != '') {
			...
			var givenName = "fileQuery";
			if(guideConf.dataSource == null || guideConf.dataSource.length == 0) givenName = null;
			formQueryDs(givenName, guideConf);
		}
		//2019.11.27
		if(guideConf.openDOlap == "false" || guideConf.openDOlap == "dataFile"){
			dbdInit();
		}
...
function formQueryDs(givenName, config){
	var type = 6;
	if (config.dataSource != null && config.ql != null && config.dataSource != '' && config.ql != ''){
		type = 2;
	} else if (config.dfxFile != '' && config.dfxFile != null) {
		type = 3;
	} else if (config.dfxScript != null && config.dfxScript != '') {
		type = 4;
	} else if (config.inputFiles != null && config.inputFiles != '') {
		type = 5;
	} else if (config.dataSource != null && config.fixedTable.length > 0) {
		type = 6;
	} else if (config.sqlId != null && config.sqlId != '') {
		type = 7;
	}
	if ((type == 0 || givenName == null) && guideConf.openDOlap != "dataFile") {
		rqAnalyse = {
			currRpx:resources.guide.js75
			,dataSets:[]
			,rpxs:[]
		...
		var d = {
			name: givenName == null ? 'new_'+new Date().getTime() : givenName
			...
			,dataId:guideConf.dataFolderOnServer+'q'+new Date().getTime()+"."+(config.dataFileType=='text'?'txt':'bin')
			,dataSource:config.dataSource == null ? "" : config.dataSource
			,ql:config.ql == null ? "" : config.ql
			,fields:null
			,parent:null
			,dfxFile:config.dfxFile == null ? "" : config.dfxFile
			,dfxScript:config.dfxScript == null ? "" : config.dfxScript
			,dfxParams:config.dfxParams == null ? "" : config.dfxParams
			,inputFiles:config.inputFiles == null ? "" : config.inputFiles
			,currTable:config.currTable == null ? "" : config.currTable
			,tableNames:config.tableNames == null ? "" : config.tableNames
			,dqlConf:{}
			,sqlId:config.sqlId == null ? "" : config.sqlId
			,rowCount:0
		};
		if(config.fixedTable != null){
			d.tableName = config.fixedTable.length>0?config.fixedTable.join(","):'';
		}
		if(rqAnalyse == null || rqAnalyse.dataSets == null || rqAnalyse.dataSets.length == 0){
			rqAnalyse = {
				currRpx:resources.guide.js75
				,dataSets:[d]
				,rpxs:[]
		...
		if(type < 6) {
			...
			if (fileExist && guideConf.showHistoryRpx == 'yes') {
				...
				var filter = "";
				if (fileExist) {
					aly.showDataSetRpxs(dsi.name,null);
				} else {
					aly.queryDataSet(1,dsi.name,function(data){
						//if (filter != '' || guideConf.currTable != '') changeFilter(filter,function(){});
						refreshStatus(dsi,function(){
		...
		$('#selectDs').append('<option value="'+d.name+'">'+d.name+'</option>');
	}
}
...
function manageDataSet(){
	...
	var dlg = art.dialog({
	    ...
	    content:'<div style="float:left;width:150px;border-right:1px solid lightgray;height:330px;overflow:auto;" id="dataSetsDiv">'
			+''
			+'</div><div id="currDataSet" style="display: inline-block;position: fixed;margin-left: 15px;width:550px;">'
			+'</div>'
	    ,ok : null
	    ,close : function() {
	...
	var dsClick = function(){
		...
		currDataSet.html(
			'<div style="margin:5px 0px 10px;"><input id="dataSetName" type="text" style="height:26px;"><input id="showDsParams" type="button" value="数据集参数" style="float:right;"><input id="saveDsBut" type="button" value="'+resources.guide.js51+'" style="float:right;margin-right: 10px;"></div>'
		...
		if (ddss1.length>0) {
			...
			ddss2 = clone(ddss1);
			if (ds.dataSource == '') {
				ddss1 = ddss1.insert(0, [resources.guide.js91]);
				ddss2 = ddss2.insert(0,[""]);
			}
		} else {
		...
		var dqlChange = function(dsName){
			...
			var selDom9 = $('<select multiple="multiple"></select>');
			var selDom9ops = getMultipleSelectOptions(tables, tableNames, ds.tableName);
			...
			$(function(){
				selDom9.multiselect();
				selDom9.multiselect('dataprovider', selDom9ops);
			});
		}
		...
		$('#showDsParams').click(function(){
			try{
				showDsParameters($('#dataSetName').val());
			}catch(e){
				console.log(e);
				alert("请先保存数据集");
			}
