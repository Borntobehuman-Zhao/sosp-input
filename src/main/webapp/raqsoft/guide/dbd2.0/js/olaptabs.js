...
function addOlapTab(name, olapPath){
	...
	$('#tabTop').append('<iframe class="olapTabContent" name="'+currentOlap+'_tab" id="'+currentOlap+'_tab"></iframe>');
	switchOlap(currentOlap);
...
function modifyOlapTab(name){
	if(name == null) return;
	var newName = pushTabName(name);
	deleteTabName(name);
	var tabDiv = $('#'+currentOlap);
	tabDiv.off('click');
	tabDiv.attr('id',newName).html(newName);
	$('#'+currentOlap+'_tab').attr('id',newName+'_tab').attr('name',newName+'_tab');
	currentOlap = newName;
...
//其他类型文件
function open2_append(){
	$('#'+currentOlap+'_tab')[0].contentWindow.openDataFile(3);
}
...
function prepare_closeBut(tabdiv){
	...
	button.click(function(){
		...
		var tabCountBeforeDelete = $('.olapTab').length;
		if(tabCountBeforeDelete == 1){
			alert('最后一页，请勿删除');
			return;
		}
