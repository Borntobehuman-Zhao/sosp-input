...
function refreshStatus(dataSet,finishFunc,container,doNotShowStatus) {
	if(dataSet.type >= 6) return;
	if(!doNotShowStatus){
		if (!container) container = $('#dsStatus');
		if (!container[0]) {
			//$('#dataHeader').append('<nav style="display:inline;float:right" id="queryStatus"></nav>');
			$('#dataHeader').find('.navbar-inner').append('<nav style="display:inline;float:right" id="queryStatus"></nav>');
...
//				}
//			}
			if(layout.dbdversion == null){
				alert("未知版本："+layout.dbdversion);
				return;
			}else if (layout.dbdversion.indexOf("1") == 0){
				alert("无法打开该版本olap文件："+layout.dbdversion);
				return;
			}
			console.log(layout);
			dbdInit(layout);
	...
	,toString : function(rqa, dbdMobileFrame) {
		if (rqa) {
			...
			return olapObj.getConfStr(null,dbdMobileFrame,this.dsWhereParams,this.sysparams,dbdStyles);
		}
	}
...
//			div4 = $(tds[7]).find('div[cType]');//param布局位置错误
			...
			div1.append("<div id='confHints' style='height:18px;width:100%;margin:3px;padding:3px;color:#a0a0a0'>"+resources.guide.js125+"</div>");
			...
			//if (conf.lefts.length == 0) 
				div2.append("<div id='confHints' style='height:200px;text-align: center; margin:3px;padding:3px;color:#a0a0a0;'>"+resources.guide.js126+"</div>");
...
//			div4.css({'min-height':'20px', 'min-width':'40px'}).find('#confHints').css('display',conf.lefts.length == 0?'block':'none');
			...
			//if (conf.fields.length == 0) 
				div3.append("<div id='confHints' style='margin:3px;padding:3px;color:#a0a0a0'>"+resources.guide.js127+"</div>");
...
//						for (var i=0; i<fields.length; i++) fields[i] = transWhereInfo(fields[i],null,false,null,field.src == fields[i]? field.calc:null);
						for (var i=0; i<fields.length; i++){
							...
							fields[i] = transWhereInfo(fields[i],null,false,null,fi_ == null ? null : fi_.exp);
						}
	...
	,generateReport : function(conf, zongji) {
		...
		var olapRpx = function() {
			...
			$.ajax({
				...
				async : true,
				url: contextPath + "/DLServletAjax?d="+new Date().getTime(),
				data: {dsj_fields:JSON.stringify(dsj_fields),action:2,dataId:aly.getDataSet(conf.dataSet).dataId,pagerStyle:0,zongji:zongji?true:false,oper:'genReport',reportId:conf.reportId,title:conf.name,rpt:JSON.stringify(rpt).replaceAll('"','<d_q>'),lefts:rpt.lefts,tops:rpt.tops,maxSize:guideConf.maxReportSize,resultRpxPrefixOnServer:guideConf.resultRpxPrefixOnServer,onlyRefreshDs:guideConf.onlyRefreshDs},
				//confs:[{type:1自定义分析报表/2模板报表,name:'报表名称',reportId:'',template:'',show:0/1,isRowData:1,lefts:[{name:'',item:itemId,use:1,order:0无序/1升序/2降序}],tops:[],fields:[],wheres:[{item:itemId}]}...] 调序、排序、改名				success: success
			});
		}
...
function saveFile(content,type,needDataPath,oldDataPath) {
	...
	var cb = function() {
		...
		$('#downloadForm #path').val($.trim($('#saveGrpxPath1').val())+"/"+name);
...
function openOlap(openType){
	openFile("olap",function(content, file){
		...
		if(openType == 0) {
			var frame = window.top.modifyOlapTab(file.substring(file.lastIndexOf('/')+1).split('.')[0]);
			//frame.src = url+"?olap="+encodeURIComponent(content==null?file:content);
			form.attr('target', "_self");
		}else if(openType == 2){
			form.attr('target', "_blank");
		}else if(openType == 1){//新标签
			var frame = window.top.addOlapTab(file.substring(file.lastIndexOf('/')+1).split('.')[0]);
			//frame.src = url+"?olap="+encodeURIComponent(content==null?file:content);
...
function openDataFile(openType){
	openFile("fileData",function(content, file){
		...
		if(!openType) {
			var frame = window.top.modifyOlapTab(file.substring(file.lastIndexOf('/')+1).split('.')[0]);
			frame.src = url+"?dfxFile=WEB-INF/files/dfx/official/readFile.dfx&dfxParams=f%3D"+encodeURIComponent(srcf);
		}else if(openType == 1){//新标签
			var frame = window.top.addOlapTab(file.substring(file.lastIndexOf('/')+1).split('.')[0]);
			frame.src = url+"?dfxFile=WEB-INF/files/dfx/official/readFile.dfx&dfxParams=f%3D"+encodeURIComponent(srcf);
		}else if(openType == 2){
			...
			var dsconfig = {
				...
				dataFileType:guideConf.dataFileType,
				dfxParams: "f="+srcf
			};
			
			formQueryDs( "new_"+new Date().getTime(), dsconfig );
		}
	});
}
...
function saveOlapTemp(){
	...
	$('#downloadForm #path').val("temp/"+name);
