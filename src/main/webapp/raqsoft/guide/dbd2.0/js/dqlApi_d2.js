...
function Olap(){
	...
	obj.getConfStr = function(layout_arg,dbdMobileFrame,dwps, sysparams, dbdStyles) {
		var gridsterWidgetRowColArray = new Array();
		var empty = new Array();
		gridsterWidgetRowColArray.push(empty);
		for(var row = 1; row <= gridsterLastRow+1; row++){
			var currRow = new Array();
			gridsterWidgetRowColArray[row] = currRow;
		}
		var areaObj = null;
		try{
			for(var l = 0; l < gridster.$widgets.length ; l++){
				var dom = $(gridster.$widgets[l]);
				var confname = dom.children().find('.singleArea').attr('confname');
				console.log(confname);
				areaObj = controlUtil.getAreaByReport(confname);
				if(areaObj == null) continue;
				areaObj.widgetHeight = dom.attr("data-sizey");
				areaObj.widgetWidth = dom.attr("data-sizex");
				areaObj.widgetCol = dom.attr("data-col");
				areaObj.widgetRow = dom.attr("data-row");
				gridsterWidgetRowColArray[areaObj.widgetRow][areaObj.widgetCol] = areaObj;
			}
		}catch(e){
			console.warn(e);
			areaObj.widgetHeight = null;
			areaObj.widgetWidth = null;
			areaObj.widgetCol = null;
			areaObj.widgetRow = null;
		}
		
		var gridsterOptions = {
			last_rows : gridsterLastRow + 1,
			divide:divide,
			defaultDivWidth:defaultDivWidth,
			defaultDivHeight:defaultDivHeight,
			defaultLiHeight:defaultLiHeight
		};
		var layout = {
				params : aly.params,
				...
				dbdStyles:dbdStyles,
				dbdversion:dbdversion,
				paramFormLayout:paramFormLayout,
				gridsterWidgetRowColArray:gridsterWidgetRowColArray,
	...
	var func4 = function(conf) {
		...
		if (conf.tops.length==0&&conf.lefts.length==0) {
			...
			if (conf.isRowData) {
				...
				for (var i=0; i<fields.length; i++) {
					rpt.cells.push({row:1,col:i+1,row2:1,col2:i+1,format:'',value:fields[i].name,valueExp:'',extend:16,leftMain:'',topMain:'',tip:5,adjustSizeMode:48,textWrap:1,hAlign:biaoti.hAlign,color:biaoti.color,backColor:biaoti.backColor,dispExp:'',formatExp:formatExp,backColorExp:transparentColor});//'230,244,255'
					rpt.cells.push({row:2,col:i+1,row2:2,col2:i+1,format:fields[i].format,value:'',valueExp:getExps(conf,fields[i],i==0?3:5,i==0?orders:''),extend:16,leftMain:'',topMain:'',tip:4,adjustSizeMode:48,textWrap:1,hAlign:zhibiao[0].hAlign,color:zhibiao[0].color,backColor:zhibiao[0].backColor,dispExp:defaultEdit.getEditExp4Rpx(fields[i].srcEdit,fields[i].dataType==1),formatExp:formatExp,backColorExp:transparentColor});
				...
				for (var i=0; i<fields.length; i++) {
					rpt.cells.push({row:i+1,col:1,row2:i+1,col2:1,format:'',value:fields[i].name,valueExp:'',extend:16,leftMain:'',topMain:'',tip:5,adjustSizeMode:48,textWrap:1,hAlign:biaoti.hAlign,color:biaoti.color,backColor:biaoti.backColor,dispExp:'',formatExp:formatExp,backColorExp:transparentColor});
				...
				for (var i=maxTop; i>=0; i--) {
					...
					for (var j=0; j<fields.length; j++) {
						...
						if (showAggrTitle || namej) {
							...
							rpt.cells.push({row:i+1,col:currCol,row2:firstAggrRow-1,col2:currCol,format:'',value:namej,valueExp:'',extend:16,leftMain:'',topMain:'',tip:7,adjustSizeMode:48,textWrap:1,hAlign:currStyle.hAlign,color:currStyle.color,backColor:currStyle.backColor,dispExp:'',formatExp:formatExp,backColorExp:transparentColor});
							var rhIdx = rowHeights1.indexOf(i+1);
							rpt.rows[rpt.rows.length] = {row:i+1,type:161,height:rhIdx==-1?defaultHeight:rowHeights2[rhIdx]};
						}
						...
						var leftMain = level[1]>0?cellCols[level[1]]+firstAggrRow:"`0";
						var topMain = level[0]>0?cellCols[conf.lefts.length+1]+level[0]:"`0";
						if (fields[j]._fieldType == 'detail') exp2 = getExps(conf,fields[j],6);
						else if (fields[j]._fieldType == 'aggr') exp2 = getExps(conf,fields[j], (fields[j].dataType == 2 || fields[j].dataType == 3) && fields[j].aggr.startsWith("m") ? 42:41);//sum还是countd
						else if (fields[j]._fieldType == 'analyse') {
							//exp = getExps(conf,fields[i],7,);
						}
						...
						if (showZongji && fields[j]._fieldType == 'aggr' && lefts.length>0) {
							for (var z=0; z<lefts.length; z++){
								var cellz = {row:firstAggrRow+z+1,col:currCol,row2:firstAggrRow+z+1,col2:currCol,format:'',value:'',valueExp:exp2,extend:16,leftMain:'',topMain:'',tip:3,adjustSizeMode:48,textWrap:1,hAlign:zongji.hAlign,color:zongji.color,backColor:zongji.backColor,dispExp:'',formatExp:'',backColorExp:transparentColor};
								rpt.cells.push(cellz);
							}
						}
					}
					if (i>0) {
						rpt.cells.push({row:i,col:lefts.length+1,row2:i,col2:fields.length==0?currCol+1:currCol,format:conf.tops[i-1].format,value:'',valueExp:getExps(conf,conf.tops[i-1],2),extend:19,leftMain:'',topMain:'',tip:2,adjustSizeMode:48,textWrap:1,hAlign:fenzu.hAlign,color:fenzu.color,backColor:fenzu.backColor,dispExp:defaultEdit.getEditExp4Rpx(conf.tops[i-1].srcEdit,conf.tops[i-1].dataType==1),formatExp:formatExp,backColorExp:transparentColor});
