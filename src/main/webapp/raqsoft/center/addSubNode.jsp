...
<%
	...
	String insideFileTable = request.getParameter( "insideFileTable" );
	...
	AbstractDefaultUserManager umi = Center.getUserManager();
...
<script language="javascript">	
	...
	function submitNode() {
		...
		if(typeBox.value == "4"){
			...
			if(analyseTypeRadiosValue == "ql"){
				...
				data1 += "&analyseDB=" + encodeURIComponent(anaDataSourceBox.value )
					...
					var chooseInsideTable = $('#selDom_insideFileTable')
					if(chooseInsideTable.length == 0 || chooseInsideTable.val() == null || chooseInsideTable.val() == ""){
						chooseInsideTable = "";
					}else{
						chooseInsideTable = chooseInsideTable.val();
					}
					var dirsValue1 = "";
					if(anaBoxInputFiles.value.charAt(0) == ';') dirsValue1 = anaBoxInputFiles.value.substring(1);
					data1 += "&anaFile=" + encodeURIComponent( dirsValue1 ) + "&chooseInsideTable="+chooseInsideTable;
		...
		if(typeBox.value == "7"){
			data1 += "&cq_json=" + encodeURIComponent(cq_json.value)
			+"&cq_dfx=" + encodeURIComponent(cq_dfx.value)
			+"&cq_rpx=" + encodeURIComponent(cq_rpx.value);
		}
...
<div class="layui-container" style="top:60px;height:100%"> 
    <div  class="layui-content" style="background-color:white">
    <div class="layui-row layui-col-space30">
    <div class="">
      <fieldset>
      <legend>基础</legend>
      <TABLE lay-skin="nob" style="border: none;" class="layui-table" cellSpacing=0 cellPadding=0 align=center>
		 	<colgroup>
		 		<col width="150px"/>
		 		<col/>
		 	</colgroup>
	...
	<legend>详细</legend>
	<TABLE lay-skin="nob" style="border: none;display: table;" class="layui-table" cellSpacing=0 cellPadding=0 align=center>
	  	<TR class="reportSpan businessInput">
	    	...
	    	<td>
	    		<input id="upReportFile" style="margin-left:-10px" type="button" class="layui-btn layui-bg-black layui-btn-sm" onclick="addReport('report_1')" value="上传"/>
	    	</td>
	    	<td>
	    		<input id="reportFileRefresh" style="margin-left:-10px" type="button" class="layui-btn layui-bg-green layui-btn-sm" onclick="refreshRaqBox(typeBox.value);" value="刷新"/>
	    	</TD></TR>
	    
	    
	  	<TR class="commonQuerySpan">
	    	<TD>元数据&nbsp;</TD>
	    	<TD><textarea id="cq_json" name="cq_json" placeholder="json"></textarea></TD>
	    </TR>
	    <TR class="commonQuerySpan">
	    	<TD>dfx&nbsp;</TD>
	    	<TD><input id="cq_dfx" name="cq_dfx" type="text"/></TD>
	    </TR>
	    <TR class="commonQuerySpan">
	    	<TD>rpx&nbsp;</TD>
	    	<TD><input id="cq_rpx" name="cq_rpx" type="text"/></TD>
	    </TR>
	    <TR class="reportSpan">
		<TD>参数表单</TD>
	    	...
	    	<td>
	    		<input style="margin-left:-10px" type="button" class="layui-btn layui-bg-green layui-btn-sm" onclick="refreshFormRpxs();" value="刷新"/>
	    	</TD>
	    </TR>
	    ...
	    <TR class="querySpan">
	    	...
	    	<td><textarea placeholder="上方选择拼接于此：value1;value2....." class="layui-textarea" style="WIDTH: 140px;" id=anaBoxInputFiles  onblur="dsInputFilesChange(this.value);"></textarea></TD></TR>
	    	<TR class="analysisSpan4 analysisSpan0">
	    	<td><span id="selDom_insideFileTable_label">填报文件中的表：</span></td><td id="selDom_insideFileTable_label_td"><select id="selDom_insideFileTable" class="layui-input" style="width: 140px;"></select></td>
	    	</TR>
...
$(function(){
	...
	if(isModify){
		...
		selDom_insideFileTable = $('#selDom_insideFileTable');
		typeBox.value = "<%=type%>";
		urlBox.value="<%=url%>";
		...
		//selDom_insideFileTable.append("<option value='"+initIftn+"'>"+initIftn+"</option>");
		if($('#anaBoxInputFiles').val() && $('#anaBoxInputFiles').val().length > 0) dsInputFilesChange($('#anaBoxInputFiles').val(),initIftn);
	...
	function initBInputDir(){
		var pos = inputHome.length + 1;
		var v = "<%=busiDir%>".substring(pos);
		$('#businessInputSubDir').val(v);
	}
	window.top.top.hideRightSideMengban();
</script>
</body>
</html>
