...
<script type="text/javascript">
	...
	function deleteSelect(){
 	...
 	window.onload = function(){
		...
		tool_bindCharCheck($('.usernameInput'),null);
 	};
 	...
 	function modifyInList(id){
		...
		if(userName.val() != null && userName.val().length != 0){
			if(userName.val() == "admin" || userName.val() == "raq_visitor" || invalidChar(userName.val())){
 	...
 	function showParams(userId){
 		//popLayuiLayer(title,contentHtml,hasOkBtn,okFunc,removeCancelBtn,width)
