...
<%
	String report = request.getParameter( "__rpx" );
	...
	String resultPage = "queryReport.jsp?__rpx=" + URLEncoder.encode( report, "UTF-8" ) + "&scroll=" + scroll;
...
<script language="javascript">
	...
	function commonQuerySubmit() {
		$('#commonReportForm').attr('action',"<%=appmap%>/reportJsp/cqReport.jsp");
		$('#commonReportForm').find('#__rpx').val('<%=report%>');
		$('#commonReportForm').find('#queryJSON').val(parent.frames["frame1"].getCommonQueryJSON());
		$('#commonReportForm').find('#params').val(parent.frames["frame1"].getParams());
		$('#commonReportForm').submit();
	}

</script>
<div style="display:none;">
	<form id=commonReportForm method=get ACTION="" target="_self">
		<input type=hidden name="__rpx" id="__rpx" value="">
